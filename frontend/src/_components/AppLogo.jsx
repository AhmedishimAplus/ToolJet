...
export default function AppLogo({ isLoadingFromHeader, className }) {
  ...
  return (
    <>
      {url ? (
        <img src={url} height={26} data-cy="page-logo" alt="Application logo" />
      ) : (
        <>{isLoadingFromHeader ? <Logo height={26} data-cy="page-logo" /> : <img src={url} className={className} alt="Application logo" />}</>
      )}
    </>
  );
}
