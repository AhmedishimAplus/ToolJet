...
input.addEventListener('keydown', (e) => {
  if (e.key === 'Enter' && !isTypingInInput()) {
    e.preventDefault();
    activeElement.click(); // Toggle to input mode
  }
});
\end{lstlisting}
...
For CodeEditor, DatePicker, and other complex input components, we ensured proper tabIndex management and ARIA attributes:

\begin{lstlisting}[caption=Complex Input Components Accessibility, label=lst:complex-input-focus]
// CodeEditor, DatePicker, etc.
<Component
  tabIndex={disabledState ? -1 : 0}
...
// DatePicker specific: Enter/Escape handling
const handleKeyDown = (e) => {
  if (e.key === 'Enter') setIsCalendarOpen(true);
  else if (e.key === 'Escape') setIsCalendarOpen(false);
};
\end{lstlisting}

CodeMirror's internal keyboard handling manages code editing shortcuts (Ctrl+Z for undo, Ctrl+F for find), while DatePicker enables Enter to open the calendar, arrow keys to navigate dates within the grid, and Escape to close the picker.
