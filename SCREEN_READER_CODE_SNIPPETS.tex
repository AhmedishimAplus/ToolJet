...
// File: BaseManageOrgConstants.jsx
const { speak } = useScreenReader();

// Announce available shortcuts on page load
useEffect(() => {
  setTimeout(() => {
...
// Announce page navigation when shortcuts are used
...
The implementation announces available keyboard shortcuts upon page initialization with a short delay to ensure the user's screen reader is ready. When users activate the shortcuts, the system announces the target page number before navigation occurs, providing immediate feedback about the navigation action.
...
const handleEditorWrapperKeyDown = (event) => {
  if (event.key === 'Enter' && !isEditMode) {
    // Navigation logic for entering edit mode
    // Announce entering edit mode
    speak(`Entering ${paramLabel}`);
  }
};

// Keymap for Tab and Escape handling
const keymap = [
  {
    key: 'Tab',
    run: () => {
      // If not in edit mode, let browser handle Tab navigation
      if (!isEditMode) {
        return false;
      }
      // Tab indentation logic when in edit mode
    }
  },
  {
    key: 'Escape',
    run: () => {
      if (isEditMode) {
        // Navigation logic for exiting edit mode
        // Announce exiting edit mode
        speak(`Exiting ${paramLabel}`);
      }
    }
  }
];
...
const handleEditorWrapperFocus = () => {
  // When wrapper gains focus via Tab
  if (!isEditMode) {
    handleFocus();
  }
};
...
<div
  ...
  // Call handleEditorWrapperFocus when focused
